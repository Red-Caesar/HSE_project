...
void Bonus(RenderWindow &window, MENU &page, Player &tank, Player &friend_t, Audio &audio,
    ...
    if (bonus_f == 1 and (tank.GetX() >= randomX - 16) and (tank.GetX() <= randomX + 16) and
        (tank.GetY() >= randomY - 16) and (tank.GetY() <= randomY + 16)) {
        audio.playBones();
        bonus_f = 2;
    ...
    if (page.TwoPlayers) {
        if ((bonus_f == 1 and friend_t.GetX() >= randomX - 16) and (friend_t.GetX() <= randomX + 16) and
            (friend_t.GetY() >= randomY - 16) and (friend_t.GetY() <= randomY + 16)) {
            audio.playBones();
            bonus_f = 3;
...
void
EnemyUpdateAndDraw(int &num_alive_enemies, float CurrentFrame, Enemy_tank t[], int &enemy_iterator, Bullet enemy_bul[],
                   std::mt19937 &engine,
    ...
    for (int i = 0; i < enemy_iterator; i++) {
        ...
        if (t[i].GetIsAlive()) {
            ...
            if (!enemy_bul[i].Is_On_f) {
                ...
                enemy_bul[i].New_Coordinates_and_Dir_Enemy(t[i]);
            }
//
            ...
            enemy_bul[i].update(g_time.GetTime());
...
///music
    ...
    while (window.isOpen()) {
        ...
        CurrentFrame += 0.005 * g_time.GetTime();
        ...
        Bonus(window, page, tank, friend_t, audio, bul, n_bul, enemies_number, bonus_f, randomX, randomY, bonus_icon,
              cur_tank_lives, tank_speed, cur_friend_tank_lives);
        OurBullets(bul, n_bul, enemy_iterator, enemy_bul, t, map, g_time, window, base_is_damaged);
        EnemyUpdateAndDraw(num_alive_enemies, CurrentFrame, t, enemy_iterator, enemy_bul, engine, map, window, g_time,
                           bul, n_bul, base_is_damaged);
